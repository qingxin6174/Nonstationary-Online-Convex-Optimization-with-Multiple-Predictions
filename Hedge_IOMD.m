function [strategy,loss] = Hedge_IOMD(T,lossfun,lossoffset)
D = sqrt(2); Lip = 2; % lossfun = @(s,t)
m = ceil(log2(1+sqrt(2)*D*T)); n = length(lossoffset)-1; e = 0.02;
delta = ones(m+1,n,n+T+1)*e; Delta = ones(n+T+1,1)*e; sigma = ones(n+T+1,1)*e;
eta = ones(m+1,n,n+T+1)/e; theta = ones(n+T+1,1)/e; vartheta = ones(n+T+1,1)/e;
X = zeros(m+1,n,n+T+1); W = ones(m+1,n,n+T+1)/(m+1)/n; omega = ones(n,n+T+1)/n;
x = zeros(m+1,n,n+T+1); w = ones(m+1,n,n+T+1)/(m+1)/n; 
...
for t = n+2:n+T+1
    h(:,t) = loss(t-1:-1:t-n);
    eta(:,:,t) = (D^2+repmat((2.^(0:m)-1)',1,n)*Lip)./sum(delta(:,:,1:t-1),3);
    X(:,:,t) = argmin(x(:,:,t),repmat(h(:,t)',m+1,1),eta(:,:,t));
    L(:,t-1) = abs(loss(t-1)-h(:,t-1));
    vartheta(t-1) = log(T)/sum(sigma(1:t-2));
